...
dotenv_values: dict[str, str] = dotenv.dotenv_values(dotenv_path=dotenv_path)
...
DEV_MODE: bool = eval(dotenv_values["DEV_MODE"])
...
if WEBHOOK_LINK is not None:
    ...
    m: re.Match[str] = re.search(r"discord(?:app)?.com/api/webhooks/(?P<id>\d{17,20})/(?P<token>[\w\.\-_]{60,68})", WEBHOOK_LINK)
    if m is None:
